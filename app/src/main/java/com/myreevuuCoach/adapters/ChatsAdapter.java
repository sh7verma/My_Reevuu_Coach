...
public class ChatsAdapter extends RecyclerView.Adapter<ChatsAdapter.ViewHolder> {
    ...
    static UnreadBatchInterface mUnreadBatchInterface;
    ...
    boolean state = false;
    int mUnReadBatchCount = 0;
    int mUnAppBatchCount = 0;

    private ChatFragment mChatFragment = null;

    public ChatsAdapter(Context mContext, int width, LinkedHashMap<String, ChatsModel> chats,
                        ArrayList<String> keys, String userId, boolean status) {
        this.mContext = mContext;
        mUtils = new Utils(mContext);
        mDb = new Database(mContext);
        this.mChatFragment = ChatFragment.Companion.getInstance();
        mScreenWidth = width;
        mChats = chats;
        mKeys = keys;
        mUserID = userId;
    ...
    public static void setUnreadBatchListner(UnreadBatchInterface listner) {
        mUnreadBatchInterface = listner;
    }

    public void setBatch(int batch) {
        mUnReadBatchCount = batch;
        mUnAppBatchCount = batch;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (state) {
            ...
            mPrivateChat = mChatFragment.getMChats().get(mChatFragment.getMKeys().get(position));
        }
        ...
        if (mPrivateChat.unread_count.get(mUserID) != 0) {
            ...
            mUnAppBatchCount = mUnAppBatchCount + mPrivateChat.unread_count.get(mUserID);

        } else {
            holder.txtUnreadCount.setVisibility(View.INVISIBLE);
        }
        mUnreadBatchInterface.onUnreadBatchCountChanged(mUnReadBatchCount, mUnAppBatchCount);
        ...
        holder.llChatOuter.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                mChatFragment.openConversationActivity(finalMPrivateChat);
            }
        });
        holder.llChatOuter.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View view) {
                mChatFragment.deleteConversation(finalMPrivateChat);
    ...
    public int getItemCount() {
        if (state) {
            ...
            return mChatFragment.getMKeys().size();
        }
    }
