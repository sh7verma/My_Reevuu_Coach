package com.myreevuuCoach.filePicker;
...
public class PickerManager {
  ...
  private int maxCount = Constants.Companion.getDEFAULT_MAX_COUNT();
  ...
  public void add(String path, int type) {
    if (path != null && shouldAdd()) {
      if (!mediaFiles.contains(path) && type == Constants.Companion.getFILE_TYPE_MEDIA()) {
        mediaFiles.add(path);
      } else if (!docFiles.contains(path) && type ==  Constants.Companion.getFILE_TYPE_DOCUMENT()) {
        docFiles.add(path);
      } else {
        return;
      }
  ...
  public void remove(String path, int type) {
    if ((type == Constants.Companion.getFILE_TYPE_MEDIA()) && mediaFiles.contains(path)) {
      mediaFiles.remove(path);
    } else if (type == Constants.Companion.getFILE_TYPE_DOCUMENT()) {
  ...
  public void addDocTypes() {
    String[] pdfs = { "pdf" };
    fileTypes.add(new FileType(Constants.Companion.getPDF(), pdfs, R.drawable.icon_file_pdf));

    String[] docs = { "doc", "docx", "dot", "dotx" };
    fileTypes.add(new FileType(Constants.Companion.getDOC(), docs, R.drawable.icon_file_doc));
...
//    fileTypes.add(new FileType(Constants.XLS, xlss, R.drawable.icon_file_xls));
    ...
    String[] txts = { "txt" };
    fileTypes.add(new FileType(Constants.Companion.getTXT(), txts, R.drawable.icon_file_unknown));
  }
